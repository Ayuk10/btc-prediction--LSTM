...
def model_statistics(predictions_test, X_test, Y_test, model):
    ...
    # Reshape predictions and Y_test to 2D arrays
    predictions_flat = predictions_test.reshape(-1, predictions_test.shape[-1])
    Y_test_flat = Y_test.reshape(-1, Y_test.shape[-1])
    
    rmse = np.sqrt(np.mean((predictions_flat - Y_test_flat) ** 2))
    ...
    # Naive prediction (predicting zero)
    naive_pred = np.zeros_like(Y_test_flat)
    rmse_naive = np.sqrt(np.mean((naive_pred - Y_test_flat) ** 2))
    mse_naive = np.mean((naive_pred - Y_test_flat) ** 2)
    mae_naive = np.mean(np.abs(naive_pred - Y_test_flat))
    r2_naive = r2_score(Y_test_flat, naive_pred)
    
    # Expected return prediction
    er_pred = np.full_like(Y_test_flat, mean_return)
    rmse_er = np.sqrt(np.mean((er_pred - Y_test_flat) ** 2))
    mse_er = np.mean((er_pred - Y_test_flat) ** 2)
    mae_er = np.mean(np.abs(er_pred - Y_test_flat))
    r2_er = r2_score(Y_test_flat, er_pred)
...
def print_model_statistics(error_statistics, X_test, Y_test, model):
    ...
    print("R²_naive: ", error_statistics["r2_naive"])
    ...
    print("R²_expected_return: ", error_statistics["r2_er"])
    
    # Optionally, you can evaluate the model on test data
    # print("\nModel evaluation on test data:", model.evaluate(X_test, Y_test))
    return
